...
<script type="text/javascript">
	...
	//getSelected和getSelections的不同性
		var rows=$("#grid").datagrid("getSelections");
		if(rows.length!=1){
		$.messager.alert("消息提示","请选择一个数据","warnning");
		}else{
			$('#customerWindow').window('open');
		}
		$.post("decideZone_noAssociateCustomer.action",function(data){
			//alert(data[0].id);
		...
		var id=rows[0].id;
		$.post("decideZone_hasAssociateCustomer.action",{"id":id},function(data){
	...
	function doDblClickRow(index,data){
		...
		$('#association_subarea').datagrid( {
			...
			rownumbers : true,
			striped : true,
			url : "decideZone_subarea.action?id="+data.id,
			columns : [ [{
				field : 'id',
				title : '分拣编号',
				width : 120,
		...
		$('#association_customer').datagrid( {
			...
			rownumbers : true,
			striped : true,
			url : "decideZone_customer.action?id="+data.id,
			columns : [[{
				field : 'id',
				title : '客户编号',
				width : 120,
...
<body class="easyui-layout" style="visibility:hidden;">
	...
	<div class="easyui-window" title="定区添加修改" id="addDecidedzoneWindow" collapsible="false" minimizable="false" maximizable="false" style="top:20px;left:200px">
		<div style="height:31px;overflow:hidden;" split="false" border="false" >
			<div class="datagrid-toolbar">
				...
				<script type="text/javascript">
					$(function(){
						$("#save").click(function(){
					    var r=$("#addDecideZoneForm").form("validate");
					    if(r){
		...
		<div style="overflow:auto;padding:5px;" border="false">
			<form id="addDecideZoneForm" method="post" action="decideZone_add.action">
				<table class="table-edit" width="80%" align="center">
					...
					<tr height="300">
						...
						<td>
							...
							="url:'subarea_listajax.action',fitColumns:true,singleSelect:false">
								<thead>  
							        <tr>  
	...
	<div class="easyui-window" title="关联客户窗口" id="customerWindow" collapsible="false" closed="true" minimizable="false" maximizable="false" style="top:20px;left:200px;width: 400px;height: 300px;">
		<div style="overflow:auto;padding:5px;" border="false">
			<form id="customerForm" action="${pageContext.request.contextPath }/decideZone_assigncustomerstodecidedzone.action" method="post">
				<table class="table-edit" width="80%" align="center">
					<tr class="title">
