    ...
    <div region="center" style="overflow:auto;padding:5px;" border="false">
       <form id="form" action="user_add.action" method="post" >
           ...
           <script type="text/javascript">
          	...
          	$.post("role_findAll.action",{},function(data){
          		for(var i=0;i<data.length;i++){
          			var id=data[i].id;
          			var name=data[i].name;
          			$("#roleid").append("<input type='checkbox' id='"+id+"'' name='roleIds' value='"+id+"'>"+"<label for='"+id+"'>"+name+"</label>");
          		}
          	...
          	$("#username").blur(function(){
          		//用户名校正
          		$.post("user_checkusername.action",{"username":this.value},function(data){
          			if(data=="1"){
          				$("#username").css("color","red");
          				$.messager.alert("消息提醒","用户名已经注册","error");
          			}else{
